...
func getDaPath(filename string) (dbPath string) {
...
func newMySQLDB() (*gorm.DB, error) {
	...
	if dsn == "" {
		...
		log.Println("using default MySQL DSN, set TEST_MYSQL_DSN in .env file or environment variable to override")
	}
...
func CleanupDB(db *gorm.DB) error {
	...
	if err := db.Exec("DELETE FROM sqlite_sequence WHERE name='posts'").Error; err != nil {
		// 忽略错误，因为表可能还没有序列记录
		_ = err
	}
	if err := db.Exec("DELETE FROM sqlite_sequence WHERE name='comments'").Error; err != nil {
		// 忽略错误，因为表可能还没有序列记录
		_ = err
	}
